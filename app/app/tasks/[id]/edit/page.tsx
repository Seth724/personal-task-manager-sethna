...
export default function EditTask() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!res.ok) { const text = await res.text().catch(()=>''); setError(`Failed to load task (${res.status}): ${text||'No body'}`); setLoading(false); return }
      const data = await res.json(); setTask(data.task); setLoading(false)
    })()
  }, [params.id])
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/tasks/${params.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    if (!res.ok) { const text = await res.text().catch(()=> ''); alert(`Update failed (${res.status}): ${text}`); return }
    router.push('/tasks')
  }
    ...
    <>
      ...
      <main className="main-wrap">
        <div className="gradient-ring card-neo p-4 md:p-6">
          <CardHeader className="px-0 pt-0"><CardTitle className="gradient-text">Edit Task</CardTitle></CardHeader>
          <CardContent className="px-0">
            <form className="grid gap-3" onSubmit={onSubmit}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <Input name="dueDate" type="date" required defaultValue={task.dueDate ? task.dueDate.slice(0,10) : ''} className="shine-on-hover" />
